...
const _hasCommandBar = (p) => {
  const props = p.commandBarProps
  return props &&
    ((props.items && props.items.length > 0) || (props.farItems && props.farItems.length > 0))
    ? true
    : false
}
  ...
  const _onRenderMenu = (): React.ReactNode => {
    ...
    if (menu || menuOther) {
      return (
        <div
          className={
            props.root ? AppViewStylesheet.rootMenuContainer : AppViewStylesheet.menuContainer
          }
        >
          {menu}
          {menuOther}
