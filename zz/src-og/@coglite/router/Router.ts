...
const notFoundHandler = (req: IRequest) => {
  return Promise.reject({
    code: "NOT_FOUND",
    request: req,
    message: `Unable to find handler for ${req.path}`
  })
}
...
export const exactPath = (handler: IRequestHandler, opts?: IExactPathOptions): IRequestHandler => (
  req,
  next
) => {
  if (
    req.basePath === req.path ||
    (opts && opts.allowTrailingSlash && req.path === `${req.basePath}/`)
  ) {
...
export const injectParametersInterceptor = (
  params: any,
...
export class Router implements IRouter {
  ...
  public use(
    pathOrRouter: string | IRouter | IRequestHandler,
...
const sansTrailingSlash = (path: string): string =>
  hasTrailingSlash(path) ? path.slice(0, -1) : path

export const pickPath = <Value>(
  path: string,
  routes: Routes<Value>,
  parentPattern = ""
): Match<Value> => {
