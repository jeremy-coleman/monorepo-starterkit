...
class PerformanceDemo extends Component {
  renderSpam = spam => <div class="spam">{spam.message}</div>
  ...
  render({ spamming, spams = [], start, stop, goInsane }) {
    ...
    return (
      <div class="demo perf">
        ...
        <header>
          <Button raised ripple onClick={stop} disabled={!spamming}>
            Stop Spamming
          </Button>
          <Button raised ripple onClick={start} disabled={spamming}>
            Start Spamming
          </Button>
          <Button raised ripple onClick={goInsane} disabled={spamming}>
            Go Insane!
          </Button>
        </header>
        ...
        <div class="spams">{visible.map(this.renderSpam)}</div>
      </div>
    )
  }
}
...
const CountDemo = connect('count', {
  ...
  <div class="demo count">
    <h2>Counter Demo:</h2>
    <h3>Count: {count}</h3>
    <Button raised ripple onClick={increment}>
      Increment
    </Button>
    <Button raised ripple onClick={reset}>
      Reset
    </Button>
  </div>
))
  ...
  <Provider store={store}>
    <div class="app">
      <header class="masthead">
        <img src="/images/stockroom.svg" />
        <h1>Stockroom</h1>
        <a class="github" href="https://github.com/developit/stockroom" target="_blank" rel="noopener noreferrer">
          Star on Github
        </a>
      </header>
      <CountDemo />
